...
{
    if(document.size() != 11 || std::count(document.begin(), document.end(), document[0]) == document.size())
        return false;
...
bool Account::verifyCnpj(const std::string& document)
{
    ...
    static const int multiplicadores1[] = {5, 4, 3, 2, 9, 8, 7, 6, 5, 4, 3, 2};
    static const int multiplicadores2[] = {6, 5, 4, 3, 2, 9, 8, 7, 6, 5, 4, 3, 2};
    ...
    for (int i = 0; i < 12; i++)
    {
        soma += (document[i] - '0') * multiplicadores1[i];
    }
    ...
    for (int i = 0; i < 13; i++)
    {
        soma += (document[i] - '0') * multiplicadores2[i];
    }
    remainder = soma % 11;
